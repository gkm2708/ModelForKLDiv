...
@register("mlp")
def mlp(num_layers=3, num_hidden=64, activation=tf.nn.relu, layer_norm=False):
    """
...
@register("encoder_mlp")
def encoder_mlp(num_layers=1, num_hidden=64, activation=tf.nn.relu, layer_norm=False):
    """
    Stack of fully-connected layers to be used as a variational encoder
    ...
    def network_fn(X):
        h = tf.layers.flatten(X)
        for i in range(num_layers):
            h = fc(h, 'encoder_mlp_fc{}'.format(i), nh=num_hidden, init_scale=np.sqrt(2))
            if layer_norm:
                h = tf.contrib.layers.layer_norm(h, center=True, scale=True)
            h = activation(h)
